...
const ProductCard = ({ data, isEvent, isCompact = false }) => {
  ...
  return (
    <>
      <div
        className={`group bg-purple-100 rounded-md ${isCompact
            ? " shadow-sm hover:shadow-md"
            : " shadow-sm hover:shadow-xl"
          } transition-all duration-300 overflow-hidden border border-gray-200 hover:border-gray-200 relative ${isCompact
            ? "hover:-translate-y-0.1"
            : "transform hover:-translate-y-0.2"
          } h-full flex flex-col ${isCompact ? "min-h-[140px]" : "min-h-[400px]"
          }`}
      >
          ...
          >
            ...
            >
              <img
                ...
                alt={data.name}
                className={`w-full h-full object-cover transition-transform duration-500 ${isCompact ? "group-hover:scale-105" : "group-hover:scale-110"
                  }`}
              />
            </div>
          </Link>
        ...
        <div
          className={`${isCompact
              ? "p-1 space-y-0 border-black "
              : "p-1.5 sm:p-2 md:p-2.5 space-y-0.5 sm:space-y-1 border-black"
            } flex-grow flex flex-col justify-between ${isCompact ? "min-h-[40px]" : "min-h-[180px]"
            }`}
        >
          {/* Top Content Section */}
          <div className="flex-grow space-y-1">
            ...
            <div className="flex items-start justify-between">
              <Link to={`/product/${data._id}`} className="flex-1">
                <h3
                  className={`font-semibold text-gray-900 hover:text-blue-600 transition-colors duration-200 line-clamp-1 leading-tight ${isCompact
                ...
                {(data.isSellerProduct ||
                  (data.sellerShop && !data.isSellerProduct)) && (
                    <div
                      className={`mt-1 ${isCompact ? "min-h-[16px]" : "min-h-[20px]"
                        }`}
                    >
                      <span className="inline-flex items-center bg-blue-100 text-blue-800 text-[8px] px-1.5 py-0.5 rounded-full font-medium">
                        ...
                        {data.isSellerProduct
                          ? data.sellerShop &&
                            ...
                            ? `${data.sellerShop.name}${data.isAdminTagged ? " (Admin Tagged)" : ""
                            }`
          ...
          <div className="space-y-1">
            {/* Price Section */}
            <div
              className={`${isCompact ? "min-h-[10px]" : "min-h-[40px]"
                } flex flex-col justify-center`}
            >
              <div className="flex items-baseline space-x-1">
                <span
                  className={`${isCompact ? "text-xs" : "text-sm sm:text-base"
                ...
                {data.originalPrice &&
                  data.originalPrice > data.discountPrice && (
                    <span
                      className={`${isCompact ? "text-[10px]" : "text-xs sm:text-sm"
              ...
              <div
                className={`${isCompact ? "text-[7px]" : "text-[10px]"
                  } text-gray-500 mt-1`}
              >
                {(() => {
                  // Check for product-specific shipping configuration first
            ...
            <div className="space-y-0.5">
              ...
              <button
                ...
                className={`w-full ${isCompact
                    ? "py-0.5 my-1 text-[7px]"
                    : "py-1 my-1 sm:py-1.5 text-[10px] sm:text-xs"
                  } rounded-md font-medium transition-all duration-200 ${data.stock < 1
                    ? "bg-gray-100 text-gray-400 cursor-not-allowed"
                    : "bg-gradient-to-r from-red-500 to-red-500 text-white hover:from-red-600 hover:to-red-700 shadow-sm hover:shadow-md transform hover:scale-[1.01] active:scale-[0.99]"
                  }`}
              >
              ...
              <button
                ...
                className={`w-full ${isCompact
                  ...
                  } rounded-md font-medium transition-all duration-200 flex items-center justify-center gap-0.5 ${data.stock < 1
                    ? "bg-gray-100 text-gray-400 cursor-not-allowed"
                    : "bg-gradient-to-r from-blue-600 to-blue-600 text-white hover:from-blue-700 hover:to-blue-700 shadow-sm hover:shadow-md transform hover:scale-[1.01] active:scale-[0.99]"
                  }`}
              >
                <RiShoppingCartFill
                  className={`${isCompact ? "w-2 h-2" : "w-2.5 h-2.5 sm:w-3 sm:h-3"
                    }`}
                />
